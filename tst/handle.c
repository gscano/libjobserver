...
int main()
{
...

  {
    ...
    assert(jobserver_terminate_job(&js, &token, false) == -1);
...

  {
    ...
    assert(jobserver_terminate_job(&js, &token, false) == -1 && errno == ECHILD);
    assert(jobserver_close(&js) == -1);
    assert(read(pipefd[0], &token, 1) == 1);
    sleep(1);
    assert(jobserver_terminate_job(&js, &token, false) == 0);
